...
impl UsernameError {
	pub fn new(nick: String, e: UsernameErrorType) -> UsernameError {
		UsernameError {
			estr: String::from(match &e {
				...
				&UsernameErrorType::Utf8(_) => format!("Invalid username: Username is not utf8: '{}'", nick),
			}),
			e: e,
		}
	}
...
impl Username {
	...
	pub fn from_bytes(bytes: Vec<u8>) -> Result<Username, UsernameError> {
		match String::from_utf8(bytes.clone()) {
			...
			Err(e) => Err(UsernameError::new(String::from_utf8_lossy(&bytes).into_owned(), UsernameErrorType::Utf8(e))),
		}
	}
